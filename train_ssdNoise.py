...
from data.augmentation import DataAugmentationGrid
import numpy as np
import torch
...
class Detector(pl.LightningModule):
    def __init__(self, stddev, **kwargs):
        ...
        self.stddev = stddev
        #self.transform = DataAugmentationGrid(self.stddev)  # per batch augmentation_kornia
        self.batch_size = 16
        self.num_epochs = 50
        self.metric = MeanAveragePrecision(iou_type="bbox", class_metrics=True)
        self.gn_stddev = stddev
    ...
    def prepare_data(self):
        self.train_dataset = DFG_Dataset(self.gn_stddev, '/graphics/scratch2/students/kornwolfd/ML_CV_practicalCourse/data_RoadSigns/dfg/', phase='train')
        self.val_dataset = DFG_Dataset(self.gn_stddev, '/graphics/scratch2/students/kornwolfd/ML_CV_practicalCourse/data_RoadSigns/dfg/', phase='test')
...
for i in np.arange(0.01, 0.12, 0.02):
    net = Detector(i)
    ...
    tb_logger = pl_loggers.TensorBoardLogger(save_dir="/graphics/scratch2/students/kornwolfd/ML_CV_practicalCourse/lightningLogs_RoadSigns", version='new_gn_grid_'+str(i))
